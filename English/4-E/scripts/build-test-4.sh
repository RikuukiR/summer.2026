#!/usr/bin/env bash
# sections/test4/ 配下のテスト PDF を生成
set -euo pipefail

ROOT="$(cd "$(dirname "$0")/.." && pwd)"
TEST_DIR="${ROOT}/sections/test4"
...
case "$TARGET" in
  confirm)
    build_test "test-main.tex" "第4回 確認テスト.pdf" "第4回 確認テスト"
    ;;
  review)
    build_test "review-main.tex" "第4回 復習テスト.pdf" "第4回 復習テスト"
    ;;
  all)
    build_test "test-main.tex" "第4回 確認テスト.pdf" "第4回 確認テスト"
    build_test "review-main.tex" "第4回 復習テスト.pdf" "第4回 復習テスト"
    ;;
  *)
    echo "用法: build-test-4.sh [confirm|review|all]" >&2
    exit 1
    ;;
esac
