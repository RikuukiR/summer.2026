% !TeX root = ../../main.tex
\subsection{助動詞とは}
    助動詞は動詞を助ける働きがあり、原則( \fitblankbf[]{動詞の前} )に置く。
...
\subsection{疑問文と否定文}
    ...
    \begin{theorembox}{\textbf{助動詞の用法（まとめ）}}
        \begin{enumerate}
            \item 助動詞は原則( \fitblankbf[]{動詞の前} )に置く
    ...
    \begin{minipage}[t]{0.48\linewidth}
        \centering
        \textbf{疑問文}\\[0.5em]
        ( \fitblankbf[]{助動詞} )が文頭に出る
    \end{minipage}\hfill
    \begin{minipage}[t]{0.48\linewidth}
        \centering
        \textbf{否定文}\\[0.5em]
        助動詞の( \fitblankbf[]{直後} )に( \fitblankbf[]{not} )をつける
    \end{minipage}
    \vspace{1.5em}

    まずは助動詞を含む文の用法を確認しておこう。
