...
\newcommand{\fitblankautounderline}[2]{%
  \sbox0{#2}%
  \setlength{\fitblankwidth}{\dimexpr\wd0+\fitblankautopad\relax}%
  \underline{\makebox[\fitblankwidth][c]{#1}}%
}
\newcommand{\fitblankunderline}[2]{%
  \sbox0{#2}%
  \ifdim\wd0<2em
    % 数字など短い答え：コンパクトな下線
    \setlength{\fitblankwidth}{\dimexpr\wd0+2\fitblanknumpad\relax}%
    \ifdim\fitblankwidth<\fitblanknummin
      \setlength{\fitblankwidth}{\fitblanknummin}%
    \fi
  \else
    % 語句など長い答え：十分な長さの下線
    \setlength{\fitblankwidth}{\dimexpr\wd0+2\fitblanksidepad\relax}%
...
\newcommand{\fitblank}[2][\relax]{%
  ...
  \else
    ...
    \else
      \ifshowanswer
        \underline{\makebox[#2em][c]{\color{blue}\ensuremath{\bm{#1}}}}%
      \else
        \underline{\makebox[#2em][c]{\phantom{\ensuremath{\bm{#1}}}}}%
      \fi
    \fi
  \fi
}
