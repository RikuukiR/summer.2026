...
\begin{exercise}
    ...
    \centering
    % 長方形の図
    \begin{tikzpicture}[scale=0.48, x=1cm, y=1cm]
        \coordinate (A) at (0,3);
        \coordinate (B) at (0,0);
        \coordinate (C) at (4,0);
        \coordinate (D) at (4,3);
        ...
        % 寸法（辺に沿った弧）
        \draw[densely dashed]
            (-0.32,3) .. controls (-0.68,1.5) .. (-0.32,0);
        \node[anchor=east] at (-0.78,1.5) {$3\ \mathrm{cm}$};
        \draw[densely dashed]
            (0,-0.32) .. controls (2,-0.68) .. (4,-0.32);
        \node[anchor=north] at (2,-0.78) {$4\ \mathrm{cm}$};
    \end{tikzpicture}

    \vspace{0.3em}

    % グラフ用の空欄
    \begin{tikzpicture}[scale=0.30, x=1.2cm, y=0.55cm]
...
\begin{proof}\mbox{}\\
    ...
    \begin{hiddenanswer}
        \noindent
        \begin{tabular}[t]{@{}p{0.48\linewidth}@{\hspace{0.8em}}p{0.48\linewidth}@{}}
        \begin{minipage}[t]{\linewidth}
        （1）$0 \leqq x \leqq 4$ のとき

        四角形 ABCP は $\mathrm{AP} /\!/ \mathrm{BC}$ の台形で，$\mathrm{AP} = x\ \mathrm{cm}$ であるから
        \[\dfrac{1}{2} \times (x + 4) \times 3 = \dfrac{3}{2}x + 6\]
        よって $y = \dfrac{3}{2}x + 6$
        \vspace{-0.3em}
        ...
        四角形 ABCP は $\mathrm{AB} /\!/ \mathrm{PC}$ の台形で，$\mathrm{PC} = 7 - x\ \mathrm{cm}$ であるから
        \[\dfrac{1}{2} \times \{(7 - x) + 3\} \times 4 = -2x + 20\]
        よって $y = -2x + 20$
        \vspace{-0.3em}
        ...
        \vspace{-0.3em}
        \noindent
        \begin{tabular}[t]{@{}l@{\hspace{1.2em}}p{0.42\linewidth}@{}}
        \begin{minipage}[t]{0.52\linewidth}
        （4）グラフより，$y = 8$ となるのは $0 \leqq x \leqq 4$ のときに $1$ 回，$4 \leqq x \leqq 7$ のときに $1$ 回ある。

        $0 \leqq x \leqq 4$ のとき，$8 = \dfrac{3}{2}x + 6$ より $x = \dfrac{4}{3}$

        $4 \leqq x \leqq 7$ のとき，$8 = -2x + 20$ より $x = 6$
        ...
        \begin{tikzpicture}[x=1cm, y=0.55cm]
            \draw[step=1, gray!40, very thin] (0,0) grid (8,14);
            \draw[thick, ->] (0,0) -- (8.5,0) node[right] {\small $x$ (秒)};
            \draw[thick, ->] (0,0) -- (0,14.5) node[above] {\small $y$ ($\mathrm{cm}^2$)};
            \node[anchor=north east] at (-0.12,-0.12) {O};
